...
func (t *IntentAnalysisTool) semanticEmbeddingClassification(ctx context.Context, input string) (string, float64, error) {
	...
	intentPrototypes := map[string]string{
		"knowledge": "查询上传文档、内部资料、项目说明和知识库内容",
		"search":    "查询互联网最新公开信息、官方文档、新闻和外部资料",
		"clarify":   "用户问题缺少必要背景，需要先补充细节",
		"general":   "一般对话咨询、解释、总结和日常沟通",
	}
...
func (t *IntentAnalysisTool) keywordMatching(input string) (string, float64) {
	...
	patterns := map[string]struct {
		...
		"knowledge": {
			keywords:   []string{"文档", "知识库", "上传", "资料", "项目", "说明", "历史", "总结"},
			confidence: 0.78,
		},
		"search": {
			keywords:   []string{"最新", "搜索", "联网", "网上", "新闻", "官方文档", "资料", "今天"},
			confidence: 0.8,
		},
		"clarify": {
			keywords:   []string{"不确定", "补充", "选择", "哪一个", "哪个", "需要更多信息"},
			confidence: 0.8,
		},
	}
...
- knowledge: 查询上传文档、内部资料、项目说明和知识库内容
- search: 查询互联网最新公开信息、官方文档、新闻和外部资料
- clarify: 问题缺少必要背景，需要先补充细节
- general: 通用对话、解释、总结、闲聊
...
func (t *IntentAnalysisTool) identifyMissingInfo(input string, intent string) []string {
	...
	switch intent {
	case "knowledge":
		if !strings.Contains(lower, "文档") && !strings.Contains(lower, "资料") && !strings.Contains(lower, "知识库") {
			missing = append(missing, "需要检索的资料范围")
		}
	case "search":
		if !strings.Contains(lower, "最新") && !strings.Contains(lower, "时间") && !strings.Contains(lower, "今天") {
			missing = append(missing, "检索时间范围")
		}
	case "clarify":
