	...
	if start >= 0 && end > start {
		...
		if err := json.Unmarshal([]byte(jsonStr), &llmResult); err == nil {
			return &OptimizationResult{
				...
				OptimizedStrategy:   buildOptimizedStrategy(strategy, llmResult.Changes, llmResult.ExpectedImprovement, "llm"),
...
func (t *OptimizeStrategyTool) optimizeWithRules(strategy, evaluation map[string]interface{}) *OptimizationResult {
	...
	return &OptimizationResult{
		OriginalStrategy:    strategy,
		OptimizedStrategy:   buildOptimizedStrategy(strategy, changes, expectedImprovement, "rules"),
		Changes:             changes,
...
func buildOptimizedStrategy(strategy map[string]interface{}, changes []string, expectedImprovement map[string]float64, source string) map[string]interface{} {
	optimized := make(map[string]interface{}, len(strategy)+1)
	encoded, err := json.Marshal(strategy)
	if err == nil {
		_ = json.Unmarshal(encoded, &optimized)
	}
	if len(optimized) == 0 && len(strategy) > 0 {
		for key, value := range strategy {
			optimized[key] = value
		}
	}
	optimized["optimization_hints"] = map[string]interface{}{
		"source":               strings.TrimSpace(source),
		"changes":              append([]string(nil), changes...),
		"expected_improvement": cloneFloatMap(expectedImprovement),
	}
	return optimized
}
