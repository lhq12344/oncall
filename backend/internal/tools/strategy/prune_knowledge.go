...
func (t *PruneKnowledgeTool) pruneKnowledge(cases []pruneCase, maxAgeDays int, minWeight float64) *PruneResult {
	...
	deleted := make(map[string]struct{}, len(deletedCases))
	for _, caseID := range deletedCases {
		deleted[caseID] = struct{}{}
	}
	for _, kcase := range cases {
		if _, ok := deleted[kcase.CaseID]; ok {
			continue
		}
		titleKey := normalizedPruneTitle(kcase.Title)
		if titleKey == "" {
			continue
		}
		if targetID, ok := mergeTargets[titleKey]; ok {
			mergedCases = append(mergedCases, kcase.CaseID)
			reasons[kcase.CaseID] = fmt.Sprintf("与案例 %s 标题相似，建议合并", targetID)
			continue
		}
		mergeTargets[titleKey] = kcase.CaseID
	}
...
func normalizedPruneTitle(title string) string {
