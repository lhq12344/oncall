...
type DeferredGatewayCatalog struct {
	mu                sync.RWMutex
...
func NewDeferredGatewayCatalog() *DeferredGatewayCatalog {
	return &DeferredGatewayCatalog{tools: make(map[string]Tool), deferred: make(map[string]bool), scopedDiscoveries: make(map[string]map[string]bool)}
}

func (r *DeferredGatewayCatalog) Register(t Tool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.tools[t.Name()] = t
...
func (r *DeferredGatewayCatalog) RegisterDeferred(t Tool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.tools[t.Name()] = t
	r.deferred[t.Name()] = true
...
func (r *DeferredGatewayCatalog) Get(name string) Tool {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.tools[name]
}

func (r *DeferredGatewayCatalog) IsDeferred(name string) bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.deferred[name]
}

const deferredDiscoverySessionKey = "tools.deferred_discovered_tools"
...
func (r *DeferredGatewayCatalog) MarkDiscovered(ctx context.Context, name string) {
...
func (r *DeferredGatewayCatalog) IsDiscovered(ctx context.Context, name string) bool {
	name = strings.TrimSpace(name)
	if name == "" {
		return false
	}
...
func (r *DeferredGatewayCatalog) ListAlways() []Tool {
	r.mu.RLock()
	defer r.mu.RUnlock()
...
func (r *DeferredGatewayCatalog) GetDeferredToolNames(ctx context.Context) []string {
...
func (r *DeferredGatewayCatalog) SearchDeferred(query string, maxResults int) []map[string]any {
...
func (r *DeferredGatewayCatalog) FindDeferredByNames(names []string) []map[string]any {
