...
func (a *EinoAdapter) InvokableRun(ctx context.Context, argumentsInJSON string, opts ...einotool.Option) (string, error) {
	...
	if info := telemetry.ContextFrom(ctx); info.Recorder != nil {
		finish = info.Recorder.StartContext(ctx, "tool.invoke", map[string]string{"tool_id": a.Tool.Name()})
	}
	result := a.Tool.Execute(ctx, args)
	if result.IsError {
		finish(fmt.Errorf("%s", result.Output))
	} else {
		finish(nil)
	}
	runPostToolHooks(ctx, a.HookEngine, a.Tool.Name(), args, result)
...
func newDefaultCatalog(ctx context.Context, checker *permissions.Checker, deferredTools ...einotool.BaseTool) *DeferredGatewayCatalog {
	return newDefaultCatalogWithHooks(ctx, checker, hooks.DefaultEngine(), deferredTools...)
}

func newDefaultCatalogWithHooks(ctx context.Context, checker *permissions.Checker, hookEngine *hooks.Engine, deferredTools ...einotool.BaseTool) *DeferredGatewayCatalog {
	fsc := NewFileStateCache()
	reg := NewDeferredGatewayCatalog()
...
// newDeferredGatewayCatalogWithHooks exposes only ToolSearch and InvokeDeferredTool.
...
func newDeferredGatewayCatalogWithHooks(ctx context.Context, checker *permissions.Checker, hookEngine *hooks.Engine, deferredTools ...einotool.BaseTool) *DeferredGatewayCatalog {
	reg := NewDeferredGatewayCatalog()
	registerDeferredAndGateway(ctx, reg, checker, hookEngine, deferredTools...)
	return reg
}

func registerDeferredAndGateway(ctx context.Context, reg *DeferredGatewayCatalog, checker *permissions.Checker, hookEngine *hooks.Engine, deferredTools ...einotool.BaseTool) {
	if reg == nil {
		return
	}
	...
	reg.Register(&ToolSearchTool{Catalog: reg})
	reg.Register(&InvokeDeferredTool{Catalog: reg, Checker: checker, HookEngine: hookEngine})
}
...
func BuildAlwaysEinoToolsWithHooks(ctx context.Context, checker *permissions.Checker, hookEngine *hooks.Engine, deferredTools ...einotool.BaseTool) []einotool.BaseTool {
	reg := newDefaultCatalogWithHooks(ctx, checker, hookEngine, deferredTools...)
...
func BuildDeferredGatewayEinoToolsWithHooks(ctx context.Context, checker *permissions.Checker, hookEngine *hooks.Engine, deferredTools ...einotool.BaseTool) []einotool.BaseTool {
	reg := newDeferredGatewayCatalogWithHooks(ctx, checker, hookEngine, deferredTools...)
	always := reg.ListAlways()
	out := make([]einotool.BaseTool, 0, len(always))
	for _, t := range always {
		out = append(out, NewEinoAdapterWithHooks(t, checker, hookEngine))
