...
func TestDeferredGatewayCatalogSearchAndInvokeRequiresDiscovery(t *testing.T) {
	t.Parallel()
	...
	invoke := &InvokeDeferredTool{Catalog: reg, Checker: permissions.NewChecker(permissions.Options{Mode: permissions.ModeBypass})}
	...
	search := &ToolSearchTool{Catalog: reg}
...
func TestDeferredGatewayCatalogDiscoveryIsScopedBySession(t *testing.T) {
	t.Parallel()
	reg := NewDeferredGatewayCatalog()
...
func TestInvokeDeferredToolAllowsSafeReadToolInDefaultMode(t *testing.T) {
	...
	invoke := &InvokeDeferredTool{Catalog: reg, Checker: checker}
	res := invoke.Execute(ctx, map[string]any{"tool_name": "k8s_monitor", "arguments": map[string]any{"value": "pod"}})
	if res.IsError || res.Output != "ok" {
...
func TestDeferredEinoToolRecordsTrace(t *testing.T) {
	sink := telemetry.NewMemorySink()
	ctx := telemetry.WithContext(context.Background(), telemetry.ContextInfo{TraceID: "trace-1", RunID: "run-1", Recorder: telemetry.NewRecorder(sink)})
	result := NewDeferredEinoTool(ctx, stubEinoTool{name: "k8s_monitor", desc: "inspect", out: "ok"}).Execute(ctx, map[string]any{"value": "pod"})
	if result.IsError {
		t.Fatalf("Execute() = %#v", result)
	}
	spans := sink.Spans()
	if len(spans) != 1 || spans[0].Name != "tool.invoke" || spans[0].Attrs["tool_id"] != "k8s_monitor" {
		t.Fatalf("unexpected trace spans: %#v", spans)
	}
}
