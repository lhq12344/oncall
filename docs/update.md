...
**核心组件**:
1. `cache.go` - 核心缓存管理器（支持 Agent 和会话隔离）
2. `llm_cache.go` - LLM 响应缓存
3. `vector_cache.go` - 向量检索缓存
4. `monitoring_cache.go` - 监控数据缓存（K8s/Prometheus/ES）
5. `eviction.go` - 缓存失效策略（TTL/LRU/条件失效）

**关键特性**:
- ✅ Agent 间上下文完全隔离（通过 AgentID + SessionID）
- ✅ 会话间缓存互不污染
- ✅ 支持多种失效策略
- ✅ 自动 TTL 管理
- ✅ 缓存统计和监控

**测试覆盖**: `test/cache_test.go`
- 缓存管理器测试
- Agent 隔离测试
- 会话隔离测试
- LLM 缓存测试
...
**使用示例**: 见 `internal/cache/README.md`
