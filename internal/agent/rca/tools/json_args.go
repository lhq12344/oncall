...
// isTruncatedJSONError 判断是否为“JSON 截断”错误。
// 输入：err（解析错误）。
// 输出：true 表示可尝试容错。
...
// unmarshalRCAArgsLenient 对 RCA 工具参数做容错解析。
// 输入：payload（原始 JSON）、target（目标结构体指针）。
// 输出：解析错误；截断 JSON 自动降级为修复后再解析。
func unmarshalRCAArgsLenient(payload string, target any) error {
	...
	repaired := repairRCATruncatedJSON(payload)
	if repaired == "" {
		repaired = "{}"
	}
...
// repairRCATruncatedJSON 尝试补齐对象/数组尾部。
// 输入：payload（原始 JSON）。
// 输出：补齐后的 JSON。
func repairRCATruncatedJSON(payload string) string {
	payload = strings.TrimSpace(payload)
	if payload == "" {
		return payload
	}
