...
| 计划验证与重规划收敛 | `ops/incident_nodes.go` | Plan/approval/verify/replan、重复问题熔断、人工升级、最终收口 |
| 对话代理工具链 | `dialogue/agent.go`, `dialogue/tools/*` | 意图分析、检索、审批执行入口 |
| 根因分析 | `rca/agent.go`, `rca/tools/*` | 证据收集、关联、推理、影响评估 |
| 命令执行 | `execution/agent.go`, `execution/tools/*` | execute_step/validate_result/rollback；不生成计划 |
...
- 高风险执行必须走审批中断：先通过 `plan_gate` 与 `plan_approval`，再由 `execute_step` 命中高风险后进入 interrupt/resume。
...
- 忽略 `verify_plan` / `replan_decider` 的重复问题重试上限，导致循环失控。
